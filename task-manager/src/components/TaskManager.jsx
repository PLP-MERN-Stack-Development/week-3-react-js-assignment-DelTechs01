...
const TaskManager = () => {
 ...
 .then((res) => res.json())
 .then((data) => {
 // Ensure no duplicate IDs by creating unique keys
 const newTasks = data.map((task) => ({
 ...task,
 uniqueId: `${task.id}-${page}`, // Unique key combining id and page
 }));
 setApiTasks((prev) => {
 // Filter out duplicates based on uniqueId
 const existingIds = new Set(prev.map((t) => t.uniqueId));
 return [...prev, ...newTasks.filter((t) => !existingIds.has(t.uniqueId))];
 });
 setLoading(false);
 })
 .catch((err) => {
 ...
 <li
 key={task.id} // Unique key for local tasks
 ...
 <li
 key={task.uniqueId} // Use uniqueId for API tasks
